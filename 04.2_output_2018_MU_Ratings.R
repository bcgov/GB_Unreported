...
# Select Flathead MU 4-01 as benchmark - see McLellan et al 2018
bench_hunterD<-UnrepLR[which(UnrepLR$MU=='4-01'),]$norm_hunterD
bench_GBD<-UnrepLR[which(UnrepLR$MU=='4-01'),]$norm_GBD
bench_roadD<-UnrepLR[which(UnrepLR$MU=='4-01'),]$norm_roadD
...
UnRep<-data.frame(WMUid=UnrepLR$WMUid,
                  MU=UnrepLR$MU,
                  LEH=UnrepLR$LEH,
...
WriteXLS(UnreportLR, file.path(dataOutDir,paste('UnReported_WMU.xls',sep='')),SheetNames=WMU_StrataL)

#Make a pretty table for GBPU
df<-UnreportLR[[1]] %>%
  dplyr::select(MU,LEH,GBDensity,RoadDensity,HunterDensity,UnReport)
colnames(df)<-c('MU','LEH','GB Density','Road Density','Hunter Density','Unreported Ratio')
...
DT<-formattable(df[order(df$MU),], list(
  MU = color_tile("white", "orange"),
  LEH = color_tile("white", "orange"),
...
export_formattable(DT,file.path(figsOutDir,"WMU_GBUNreportedTable.png"))
